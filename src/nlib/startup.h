...
//       narray_init_memory();
//       narray_remap0();   // RAM-model builds only (lib/remap_g4.h)
...
// call to narray_remap0() from lib/remap_g4.h, a G4-only unit: the app
...
extern uint32_t _sifast, _sfast, _efast;                  // NARRAY_FAST copy span (VMA == LMA no-op where FAST is flash)

// NARRAY_FAST places a function in the FAST region — ITCM RAM on F7/H7
// (copied at boot by narray_init_memory), plain flash on G4. For the
// deadline-handler class: IRQ bodies whose worst case must not depend on
// flash wait states or cache warmth.
#define NARRAY_FAST __attribute__((section(".fast")))

// NARRAY_DMABUF places a buffer in the DMARAM region — an SRAM every DMA
...
#define NARRAY_DMABUF __attribute__((section(".dmabuf")))
...
static inline void narray_init_memory(void) {
	...
	// NARRAY_FAST code to its run home (ITCM); empty or VMA == LMA elsewhere
